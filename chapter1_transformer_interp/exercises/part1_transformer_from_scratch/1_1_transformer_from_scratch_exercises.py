# -*- coding: utf-8 -*-
"""1.1_Transformer_from_Scratch_exercises.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ZociawrnVGSomi_g6ZE3VW4pABjmYscH
...
class LayerNorm(nn.Module):
    ...
    def forward(self, residual: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
      mean = residual.mean(dim=-1, keepdim=True) #shape = (batch, posn, 1)
      var = residual.var(dim=-1, keepdim=True, unbiased=False) #shape = (batch, posn, 1)

      #normalize part
      normalized = (residual - mean) / t.sqrt(var + self.cfg.layer_norm_eps)

      #scaling and shift (broadcast weight and bias to match the shape)
      out = self.w * normalized + self.b #shape = (batch posn d_model)
...
class Embed(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_model"]:
      #tokens.
      #look up the corresponding embedding vectors
      #nn.Embedding
...
class PosEmbed(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_model"]:
      ...
      #from w_pos, first seq_len positional embeddings
      pos_embeds = self.W_pos[:seq_len]

      #broadcast across the batch dimension
      output = pos_embeds.unsqueeze(0).expand(tokens.shape[0], -1, -1) #shape "batch position d_model"
...
class Attention(nn.Module):
    ...
    def apply_causal_mask(
        ...
        """
        query_pos_t = attn_scores.shape[2]
        key_pos_t = attn_scores.shape[3]
        batch_t = attn_scores.shape[0]
        n_heads_t = attn_scores.shape[1]

        #sequence length query position = key position
        seq_len = attn_scores.size(-1)

        #create mask: true when query wants to look forward (future) i < j
        mask = t.triu(t.ones((query_pos_t, key_pos_t), device=device), diagonal=1).bool() #shape (query_pos, key_pos) broadcastable over batch and n_heads
        mask = mask.unsqueeze(0).unsqueeze(0).expand(batch_t, n_heads_t, -1,-1)

        #put mask on attention scores
...
class Attention(nn.Module):
    ...
    def forward(self, normalized_resid_pre: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
      ...
      # B, P, D = normalized_resid_pre.shape #batch, position, d_model
      # H = self.cfg.n_heads
      d_head = self.cfg.d_head

      #step 1: project input to Q, V, K
      #shape = (batch, posn, head, d_head)
      q = t.einsum('bpd, hdf -> bphf', normalized_resid_pre, self.W_Q) + self.b_Q
      k = t.einsum('bpd, hdf -> bphf', normalized_resid_pre, self.W_K) + self.b_K
      v = t.einsum('bpd, hdf -> bphf', normalized_resid_pre, self.W_V) + self.b_V

        # #step 2: calculate attention
      attn_scores = t.einsum('bphf, bphf -> bhpq', q, k) / (d_head ** 0.5)

            #step 3: apply casual mask
      attn_scores = self.apply_causal_mask(attn_scores)

      #step 4: softmax over key_pos (last dimension)
      attention_pattern = attn_scores.softmax(dim=-1)
    ...
    #   attn_scores_masked = self.apply_causal_mask(attn_scores / self.cfg.d_head**0.5)
      ...
      return attn_out
      # #step 2: calculate attention
      # attn_scores = t.einsum('bphf, bqhf -> bhpq', q, k) / (d_head ** 0.5)
      ...
      # #step 5: use attn pattern to mix values.
      # #b, h, qpos, dhead = (attn @ v)
      # z = t.einsum('bhpq, bphf -> bphf', attention_pattern, v)

      # #step 6: output projection (merging heads into d_model)
      # #shape (batch, posn, head, d_head) -> (batch, posn, d_model)
      # output = t.einsum('bphf, hfd -> bpd', z, self.W_O) + self.b_O
...
class MLP(nn.Module):
    ...
    def forward(self, normalized_resid_mid: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
      #linear layer from d_model to d_mlp
      pre_activation = normalized_resid_mid @ self.W_in + self.b_in #shape [batch, posn, d_mlp]
      #apply the nonlinear function using GELU
      activated = gelu_new(pre_activation)

      #projecting back to d_model
      post_activation = activated @ self.W_out + self.b_out #shape  batch posn d_model
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, resid_pre: Float[Tensor, "batch position d_model"]) -> Float[Tensor, "batch position d_model"]:
      ...
      normalized_resid1 = self.ln1(resid_pre)

      #step 2: multihead attention
      attn_out = self.attn(normalized_resid1)

      #step 3: residual connection
      resid_mid = resid_pre + attn_out

      #step 4: normalize again before MLP
      normalized_resid2 = self.ln2(resid_mid)

      #step 5 mlp
      mlp_out = self.mlp(normalized_resid2)

      #step 6 final residual
      resid_post = resid_mid + mlp_out #shape batch position d_model
...
class Unembed(nn.Module):
    ...
    def forward(
      ...
      #linear projection from d_model to basically get d_vocab by adding in weights and biases
...
class DemoTransformer(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_vocab"]:
        ...
        token_emb = self.embed(tokens) #batch position d_model
        pos_emb = self.pos_embed(tokens)

        #adding them together to get starting resid stream
        resid = token_emb + pos_emb
...
class TransformerTrainer:
    ...
    def training_step(self, batch: dict[str, Int[Tensor, "batch seq"]]) -> Float[Tensor, ""]:
        ...
        """
        #  getting the tokens and setting up logits
        tokens = batch["tokens"].to(device)
        logits = self.model(tokens)

        #compute the log probabilities
        log_probs = get_log_probs(logits, tokens) #shape: [batch seq]
    ...
    def evaluate(self) -> float:
        ...
        for batch in self.test_loader:
          ...
          correct += (predicted_tokens == tokens[:, 1:]).sum().item()
...
class TransformerSampler:
    ...
    def sample(self, prompt: str, max_tokens_generated=100, verbose=False, **kwargs) -> str:
        ...
        for _ in range(max_tokens_generated):
          logits = self.model(input_ids[None, :])[:, -1,:].squeeze(0) #[d_vocab]
          next_token = self.sample_next_token(input_ids, logits, **kwargs)
          input_ids = t.cat([input_ids, t.tensor([next_token],device=device)])
          if verbose:
    ...
    def sample_basic(logits: Float[Tensor, "d_vocab"]) -> int:
        ...
        """
        probs = logits.softmax(dim=-1)
        return int(t.multinomial(probs, num_samples=1).item())
    ...
    def sample_top_p(logits: Float[Tensor, "d_vocab"], top_p: float, min_tokens_to_keep: int = 1) -> int:
        ...
        """
        #sorting the logits in descending order
        sorted_logits, sorted_indices = logits.sort(descending=True)

        #getting cum sum over the sorted logits
        cumulative_probs = sorted_logits.softmax(dim=-1).cumsum(dim=-1)

        #set up a cutoff
        cutoff = cumulative_probs > top_p
        if cutoff.any():
          cutoff_idx = max(cutoff.nonzero()[0].item() + 1, min_tokens_to_keep)
          sorted_logits = sorted_logits[:cutoff_idx]
          sorted_indices = sorted_indices[:cutoff_idx]

        probs = sorted_logits.softmax(dim=-1)
        output = sorted_indices[t.multinomial(probs, num_samples=1)].item()
        return int(output)
    ...
    def beam_search(
        ...
        num_return_sequences: int, #numb of sequene to return
        num_beams: int,
        max_new_tokens: int, #max to generate of tokens
        no_repeat_ngram_size: int | None = None, #if this is set, prevent generating the same n-gram twice
        ...
        """
        input_ids = self.tokenizer.encode(prompt, return_tensors="pt").to(device)[0]
        beams = [(0.0, input_ids)] #each beam is made up of (score, tokens)
...
class Beams:
    ...
    def generate(self, k: int, no_repeat_ngram_size: int | None = None) -> "Beams":
        ...
        #pass the tokens through the model to get logits for the next token
        logits = self.model(self.tokens)[:, -1, :] #shape (batch, vocab)
        logprobs = logits.log_softmax(-1) #softmax over last dim


        #pass
        #apply no-repeat ngram contraint if specified
        if no_repeat_ngram_size is not None and no_repeat_ngram_size > 0:
          for i in range(batch_size):
            token_list = self.tokens[i].tolist()
            ngram_prefix = get_last_ngram(token_list, no_repeat_ngram_size - 1)
            if ngram_prefix:
              #build a set of all previous ngrams
              blocked_tokens = {
                  token_list[j + no_repeat_ngram_size - 1]
                  for j in range(len(token_list) - no_repeat_ngram_size -1)
                  if tuple(token_list[j:j + no_repeat_ngram_size -1]) == ngram_prefix
              }
              logprobs[i, list(blocked_tokens)] = -float("inf")

        #get top-k token extensions for each sequence
        topk_logprobs, topk_indicies = logprobs.topk(k=k, dim=-1)

        #expand and combine tokens and logprobs
        new_tokens = []
        new_logprobs = []

        for i in range(batch_size):
            for j in range(k):
                new_seq = torch.cat([self.tokens[i], topk_indices[i, j].unsqueeze(0)], dim=0)
                new_tokens.append(new_seq)
                new_logprobs.append(self.logprob_sums[i] + topk_logprobs[i, j])

        return Beams(
            self.model,
            self.tokenizer,
            logprob_sums=torch.stack(new_logprobs, dim=0),
            tokens=torch.stack(new_tokens, dim=0),
        )
    ...
    def filter(self, k: int) -> tuple["Beams", "Beams"]:
        ...
        for i in range(len(self.tokens)):
            if self.tokens[i, -1].item() == eos_token_id:
                terminated.append(i)
            else:
                ongoing.append(i)

        # Sort both sets of beams
        all_indices = torch.arange(len(self.tokens))
        topk_logprobs, topk_indices = self.logprob_sums.topk(k)

        topk_masked = topk_indices.tolist()

        final_terminated = [i for i in topk_masked if i in terminated]
        final_ongoing = [i for i in topk_masked if i in ongoing]

        return self[final_ongoing], self[final_terminated]
